...
<html lang="en-GB">
  ...
  $item = $stmt2->fetchAll(PDO::FETCH_ASSOC);
    ...
    .removeAttr('href');
      ...
      function BJS_BUYITEM() {
        window.location.href = 'https://www.baratis.tk/api/buyItem?item=<?php echo $id; ?>';
        }
  ...
  <body>
      ...
      if($stmt->rowCount() == '1') {
        ...
        ?>
        <main-header-middle>Are you sure you want to buy <?php echo $item[0]['itemName']; ?> for <?php echo $item[0]['price']; ?> RealBux?</main-header-middle>
        <center><br>
    ...
    <center class="margin-top-20">
        <purchase-option onclick="BJS_BUYITEM();" class="margin-right-5"><button class="button button-blue">Buy</button></purchase-option><purchase-option onclick="BJS_CLOSEWIN();" class="margin-left-5"><button class="button button-grey">Cancel</button></purchase-option>
      </center>
          <!-- </div> !-->
    </body>
  </html>